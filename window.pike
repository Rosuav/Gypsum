...
class zadvoptions
{
	...
	mapping(string:mapping(string:mixed)) items=([
		...
		#define COMPAT(x) " Requires restart."+(has_index(all_constants(),"COMPAT_"+upper_case(x))?"\n\nCurrently active.":"\n\nCurrently inactive.")+"\n\nYou do NOT normally need to change this.","type":"int","path":"compat/"+x,"options":([0:"Autodetect"+(G->compat?({" (disable)"," (enable)"})[G->compat[x]]:""),1:"Enable compatibility mode",2:"Disable compatibility mode"])
