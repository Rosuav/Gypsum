...
class zadvoptions
{
	...
	mapping(string:mapping(string:mixed)) items=([
		...
		"Compat: Pause key":(["desc":"On some systems, the Pause key generates the wrong key code. If pressing Pause doesn't pause scrolling (or if other keys do), enable this to use Ctrl-P exclusively."COMPAT("pausekey")]),
		...
		"Hide input":(["path":"window/hideinput","type":"int","desc":"Local echo is active by default, but set this to disable it and hide all your commands.\n\nThis has nothing to do with password mode, wherein the server requests that echo be temporarily disabled. Gypsum responds to that by masking input in the normal way for passwords.","options":([0:"Disabled (show commands)",1:"Enabled (hide commands)"])]),
		...
		"Internet protocols":(["path":"connection/protocol","default":"","desc":"Gypsum can use IPv4 and/or IPv6 to establish connections, but you may choose to disable one or other.\n\nAvoid the 'direct connection' option unless you understand what it does - read the source. It can seriously lag out your connections.","options":(["":"Both (first to respond is used)","4":"IPv4 only","6":"IPv6 only","*":"{DEBUG} Direct connections"])]),
