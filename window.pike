...
class connect_menu
{
	...
	void load_content(mapping(string:mixed) info)
	{
		if (!info->port) {info->port=23; win->port->set_text("23");} //Is it worth promoting default values to core? "#Port=23" (and this should go before the colon, if there is one)
